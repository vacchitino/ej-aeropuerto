...
public void setAsientosOcup(int asientosOcup) {
    if(asientosOcup < 0) {
        throw new IllegalArgumentException("No se pueden asignar asientos negativos.");
    }else if (asientosOcup > maxPasajeros) {
        throw new IllegalArgumentException("No se puede superar la capacidad máxima del avión.");
    }else {
        this.asientosOcup = asientosOcup;
    }   
}
public int verificarDisp() {
    return maxPasajeros - asientosOcup;
}
